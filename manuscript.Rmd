---
title: "Depth over distance: Hawaiian cryptic reef communities are more distinct across ~90 m depth than ~2500 km of shallow habitat"
author:
...
We assessed variation among groups using `betadisper` and `adonis2`. These functions implement analysis of multivariate homogeneity of group dispersions (PERMDISP) and multivariate permutational analysis of variance (PERMANOVA) respectively. PERMDISP is used to assess variability within groups and PERMANOVA is used to assess differences among groups. Significance was assessed with permutation tests in both cases. Variation among sampling groups (island group and depth) was further examined and visualized using canonical analysis of principal coordinates (CAP) based on discriminant analysis [@andersonCanonicalAnalysisPrincipal2003] as implemented in the `CAPdiscrim` function in `BiodiversityR` [@kindtTreeDiversityAnalysis2005]. 
...
Environmental variables were strongly clustered according to island group (MHI vs NWHI), with the separation primarily along the direction influenced by human impact, coral cover, sea-surface temperature, and chlorophyll ([@fig:fig_pca]). The best dbRDA model (`r permanova(db_modl,stat="F")`) explained `r pc(RsquareAdj(db_modl)$adj.r.squared,0.01)` (adjusted $\mathrm{R}^2$) of the variability in crab communities across the archipelago ([@fig:fig_dbrda]). `r str_to_sentence(english(nterm))` of the seven environmental variables tested were significant and sufficiently uncorrelated. The two drivers contributing most to explained variation were `r drivers$term[1]` (F = `r num(drivers$stat[1],0.01)`) and `r drivers$term[2]` (F = `r num(drivers$stat[2],0.01)`) ([@tbl:rda_table]). <!--TODO: perhaps include discussion of the vector lengths here as well, since chl-a is the shortest vector despite being the second most-significant factor.-->
...
# Discussion {-}
Marine communities are structured by a suite of factors, but there remains no consensus for generalized mechanisms for coral reefs<!-- structuring communities in shallow coral reefs-->. Classic studies from the intertidal point to biotic factors determining lower limits and abiotic factors setting upper limits of vertical species distributions [@connellEffectsCompetitionPredation1961; @connellInfluenceInterspecificCompetition1961; @dennyPhysicalProcessesThat2001; @paineIntertidalCommunityStructure1974]. However, these conclusions describe distributions at individual sites and generalizations about the mechanisms and ecological processes that structure coral reef communities remain a subject of debate [@bellwoodEnvironmentalGeometricConstraints2005; @bellwoodSleepingFunctionalGroup2006; @carrBiodiversityPopulationRegulation2002; @connollyIndoPacificBiodiversityCoral2003; @dornelasCoralReefDiversity2006; @grahamImportanceStructuralComplexity2013; @komyakovaRelativeImportanceCoral2013]. In the Hawaiian archipelago, we found significant differences in the composition of cryptic crab communities between the populated high islands of the MHI and the uninhabited atolls of the NWHI. Moreover, these differences were driven by a combination of environmental (sea-surface temperature, chlorophyll concentration, depth), biotic (potential larval immigration), and anthropogenic (cumulative human impacts) factors. <!--Alpha diversity metrics show slight differences in the number of species and overall abundance of crabs in each sampled assemblage between the Main and Northwestern Hawaiian Islands, but CAP and PERMANOVA highlight significant differences in species assemblages between the regions. Differences in the species assemblages across regions are driven primarily by human impacts, cholorophyll a and sea surface temperature, although other factors such as larval connectivity, distance to nearest island, mean coral cover, and depth are also significant.--> 

The Hawaiian Archipelago is among the most remote on the planet and encompasses a range of gradients (anthropogenic, biotic, abiotic), making it an ideal area to study the impacts of these factors on species assemblages across space. Human impacts are considerably higher in the populated MHI than in the protected and remote NWHI [@selkoeEvaluatingAnthropogenicThreats2008; @selkoeMapHumanImpacts2009; @selkoeDNACoralReef2016]. Although we did not consider individual effects here, an index representing cumulative anthropogenic impacts was the top driver of brachyuran community structure, similar to findings in previous work examining genetic diversity across the archipelago [@selkoeDNACoralReef2016]. The next-most significant driver, chlorophyll a, can be used as a proxy for primary productivity. Productivity gradients have been reported across the Hawaiian Archipelago, which coincide with differences in coral reef community composition [@selkoeDNACoralReef2016], endemic reef fish dominance [@friedlanderDominanceEndemicsReef2020], and survival rates of top predators [@bakerEffectVariableOceanic2007]. It is not surprising, therefore, that we find differences in the brachyuran crab communities are driven by these same factors across the Hawaiian archipelago.

Despite the fact that shallow ARMS units were deployed within a narrow depth range (8--17 m), depth still emerged as a significant driver of community variation. Depth in and of itself does not affect anything [[cite?]]{.mark}, but at the same time, the factors that are known to covary most consistently with it (viz. temparature and light) do not show much variation across such a narrow range. However, we also did not sample these values for the specific depths where units where deployed. So, depth is clearly doing something. We only have environmental data from units deployed \< 30 m, but community ordinations show that there are considerable differences between all the shallow units across the entire span of the archipelago and the $\geq$ 30 m units collected from a single island.
...
```{r fig-alpha2, fig.cap='Box plots of (a) species richness and (b) Simpson diversity by island group for shallow samples only. Notches as in [@fig:fig_alpha].', results="asis", dev="svg", fig.width=8, fig.height=4, dependson="init"}
...
ggplot(bp,aes(x=PC1,y=PC2)) + 
  ...
  stat_ellipse(geom="polygon",aes(group=island_group,fill=island_group),color="black",alpha=0.1,show.legend = FALSE, size=0.07) +
...
```{r pairwise-island, fig.cap='Significance levels of pairwise PERMANOVA tests by island for shallow crab communities (p-values adjusted according to the false discovery rate [@benjaminiControllingFalseDiscovery1995]).', results="asis", dev="svg", fig.width=12, fig.height=9, dependson="init"}
...
print_table(nasa_table,"nasa_table","NASA data repository filenames for CHL and SST satellite data",accuracy=1) 
```
