...
Environmental variables were strongly clustered according to island group (MHI vs NWHI) (Supplementary [@fig:fig_supp_pca]). The best dbRDA model (`r permanova(db_modl,stat="F")`) explained `r pc(RsquareAdj(db_modl)$adj.r.squared,0.01)` (adjusted $\mathrm{R}^2$) of the variability in crab communities across the archipelago ([@fig:fig_dbrda]). `r str_to_sentence(english(nterm))` of the `r str_to_lower(english(nrow(model_terms)))` environmental variables tested were significant and sufficiently uncorrelated. The two drivers contributing most to explained variation were `r drivers$term[1]` (F = `r num(drivers$stat[1],0.01)`) and `r drivers$term[2]` (F = `r num(drivers$stat[2],0.01)`) (Supplementary [@tbl:tbl_supp_rda]). These factors also produced the longest effect vectors in the dbRDA plot ([@fig:fig_dbrda]).
...
print_table(model_terms,"tbl_env","Potential environmental drivers of crab community structure across the Hawaiian Archipelago",hide="variable")
```
...
(p1 + p2) &
  ...
  theme(plot.tag = element_text(face="italic"))
cat("{#fig:fig_supp_alpha2}")
```

\newpage

```{r fig-supp-pca, fig.cap='Principal component analysis (PCA) of environmental variables by sampling site. Ellipses represent 95% confidence intervals around island groups.', results='asis', dev='svg', fig.width=8,fig.height=8, dependson='init'}
...
cap_var <- list(x = cap_var[1], y = cap_var[2])
```
```{r fig-supp-cap-reduced, fig.cap="CAP ordination of crab communities by island group and depth zone showing only samples deployed during the years 2010--2013.", results="asis", dev="svg", fig.width=9, fig.height=6, dependson="cap-analysis-reduced"}
# make the sample group look nice
...
ggplot(pcap_reduced,aes(x=LD1,y=LD2,fill=sample_group)) +
  ...
  ylab(str_glue("Linear discriminant 2 [{pc(cap_var$y,0.1)}]")) +
  plotz_theme()
cat("{#fig:fig_supp_cap_reduced}")
```

\newpage

```{r fig-supp-euclid, fig.cap='Box plot of Euclidean distances between points in CAP ordination by depth zone. Asterisk indicates significant difference (Welch two-sample t-test).', results="asis", dev="svg", fig.width=6, fig.height=4, dependson="cap-analysis"}
dists <- bind_rows(
...
```{r tbl-supp-rda,results="asis",dependson="rda-analysis"}
print_table(rda_table,"tbl_supp_rda","Significant marginal terms in the dbRDA analysis of crab community variation across the Hawaiian Archipelago",hdr_case="none",accuracy=0.001)
```
