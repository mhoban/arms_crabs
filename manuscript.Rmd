...
permanova <- function(modl,d=1) {
  ...
  if (nrow(m) > 1) {
    m %>%
      pmap_chr(~{
        with(list(...), {
          str_glue("{term}: DF = {round(df,1)}, SS = {round(SumOfSqs,1)}, $\\mathrm{{R}}^2$ = {round(R2,1)}, pseudo-F = {round(statistic,1)}, {pval(p.value)}")
        })
      }) %>%
      str_c(collapse = "; ")
  } else {
    with(m %>% slice(1),{
      str_glue("DF = {round(df,1)}, SS = {round(SumOfSqs,1)}, $\\mathrm{{R}}^2$ = {round(R2,1)}, pseudo-F = {round(statistic,1)}, {pval(p.value)}")
    }) 
  }
}
...
print_table <- function(tbl,id="",caption="",hdr_case="sentence") {
  if (!grepl("^: ",caption)) {
...
```{r site-map, fig.cap='Sampling map (Papahānaumokuākea Marine National Monument shaded in light blue)', results="asis", dev="svg", fig.width=9, fig.height=6, dependson="init"}
library(sf)
...
```{r alpha-box, fig.cap='Box plots of Simpson diversity by island group. Notches show nonparametric confidence intervals about the median', results="asis", dev="svg", fig.width=10, fig.height=8, dependson="init"}
...
# PERMANOVA of 
anova_region <- adonis2(cc$crab_dist_shallow ~ island_group,data=cc$crab_data_shallow,permutations=9999)
anova_island <- adonis2(cc$crab_dist_shallow ~ island,data=cc$crab_data_shallow,permutations=9999)
```

There were significant differences in shallow crab community structure between the Main (MHI) and Northwest Hawaiian Islands (`adonis2` PERMANOVA: `r permanova(anova_region)`) as well as among individual islands (`r permanova(anova_island)`; see [@fig:pairwise_island] for results of pairwise comparisons).
...
```{r pairwise-island, fig.cap='Results of pairwise PERMANOVA tests by island (FDR-adjusted p-values)', results="asis", dev="svg", fig.width=12, fig.height=9, dependson="init"}
library(ggtext)
pairwise_island <- pairwise_adonis(cc$crab_dist_shallow,cc$crab_data_shallow$island,permutations=999)
...
pw1 <- pairwise_island %>%
  ...
  mutate(sig = p_adj < 0.05, p_adj = pval(p_adj)) %>%
  mutate(
...
ggplot(pw1) + 
  # geom_tile(aes(x=right,y=left,fill=sig),width=0.6,height=0.8) + 
  # geom_tile(aes(x=right,y=left),width=0.6,height=0.8) + 
  geom_richtext(
    aes(
      x=right,
      y=left,
  ...
  # scale_fill_manual(values = c("white","lightgoldenrod")) +
  guides(fill="none") +
  xlab("") + ylab("") +
  scale_x_discrete(position="top") +
  scale_y_discrete(limits=rev)
cat("{#fig:pairwise_island}")
```