...
format_list <- function(l) {
  l[length(l)] <- str_c("and ",l[length(l)])
  if (length(l) > 2) { 
    str_c(l,collapse=", ")
  } else {
    str_c(l,collapse=" ")
...
crabbo <- cc$crabs %>%
  ...
  group_by(family) %>%
  filter(n_distinct(shallow_deep) == 1) %>%
  summarise(zones = str_c(unique(depth_zone),collapse=", "), deeps = str_c(unique(shallow_deep),collapse=", "),species=str_c(unique(species),collapse=", ")) %>%
  ungroup() %>%
...
crabbo <- cc$crabs %>%
  ...
  ungroup() %>%
  arrange(deeps,zones,species) %>%
  select(deeps,species,zones) %>%
  mutate( deeps = deep_map[deeps] ) %>%
...
print_table(nasa_table,"nasa_table_s","NASA data repository filenames for CHL and SST satellite data",accuracy=1,hdr_case=NA,format_numbers=FALSE) 
```
