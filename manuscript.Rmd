...
format_species.data.frame <- function(spp,authority=TRUE,...) {
  spp %>%
    pwalk(~{
      row <- list(...)
      # s <- list(found = row$found, authority=row$authority,otu=row$otu)
...
pval <- function(p) {
  case_when(
    p < 0.001 ~ "*p* \\< 0.001",
    p < 0.01 ~ "*p* \\< 0.01",
    TRUE ~ as.character(str_glue("*p* = {round(p,2)}"))
  )
}

permdisp <- function(bd,d=1,perm=999) {
  a <- anova(bd,permutations=perm) %>%
    broom::tidy() %>%
    filter(term == "Groups")
  with(a,str_glue("DF = {round(df,d)}, SS = {round(sumsq,d)}, F = {round(statistic,d)}, {pval(p.value)}"))
}

permanova <- function(modl,d=1,stat="pseudo-F",by=NULL) {
  has_r2 <- TRUE
  if (is(modl,"dbrda")) {
  ...
  if (nrow(m) > 1) {
    m %>%
      pmap_chr(~{
        with(list(...), {
          if (has_r2) {
            r2 <- str_glue(" $\\mathrm{{R}}^2$ = {round(R2,d)},") 
          } else {
            r2 <- ""
          }
          str_glue("{term}: DF = {round(df,d)}, SS = {round(SumOfSqs,d)},{r2} {stat} = {round(statistic,d)}, {pval(p.value)}")
        })
      }) %>%
      str_c(collapse = "; ")
  } else {
    with(m %>% slice(1),{
      ...
      str_glue("{term}: DF = {round(df,d)}, SS = {round(SumOfSqs,d)},{r2} {stat} = {round(statistic,d)}, {pval(p.value)}")
      # str_glue("DF = {round(df,d)}, SS = {round(SumOfSqs,d)}, $\\mathrm{{R}}^2$ = {round(R2,d)}, pseudo-F = {round(statistic,d)}, {pval(p.value)}")
    }) 
  }
}
...
print_table <- function(tbl,id="",caption="",hdr_case="sentence",hide=character(0)) {
  if (!grepl("^: ",caption)) {
...
![An Autonomous reef monitoring structure (ARMS) unit deployed on a reef in the Red Sea (photo credit: Michael Berumen, used with permission)](data/images/berumen_uncropped.png){#fig:fig_arms}
...
splained <- function(name,constrained,total,accuracy=0.1) {
  str_glue("{name} ({scales::percent(constrained,accuracy)} constrained, {scales::percent(total,accuracy)} total)")
}
...
Shallow crab community structure was significantly different between the MHI and NWHI (`r permanova(anova_region,2)`) as well as among individual islands (`r permanova(anova_island,2)`; see [@fig:fig_pairwise] for results of pairwise comparisons). There was no significant difference in community dispersion between the MHI and NWHI (`r permdisp(bd_region)`). When mesophotic ($\geq$ 30 m) units were included, there were significant differences among depth zones (\< 30 m, 30 m, 60 m, 90 m) (`r permanova(anova_depth,2)`) and community dispersion (`r permdisp(bd_depth)`). However, community dispersion was not different among depth zones $\geq$ 30 m (`r permdisp(bd_deep)`). Although crab communities varied significantly among both depth zones and island groups (`r permanova(anova_both,2)`), the effect size (model $\mathrm{R}^2$) of depth was `r num(effect_ratio,0.1)` times greater than that of island group. In the CAP analysis, samples were successfully assigned to their respective sampling groups `r scales::percent(cap_depth$percent/100,0.1)` of the time. The first two axes explained `r scales::percent(cap_var$x,0.1)` and `r scales::percent(cap_var$y,0.1)` of the variation, respectively. The samples from the three mesophotic groups on O&#x02BB;ahu grouped separately from the shallow samples collected across the entire ~2,500 km of the Hawaiian Islands ([@fig:fig_cap]). In addition to grouping apart from shallow samples, there was as much or more variation across the 30--90 m samples than across all of the shallow samples, although that variation was largely due to the differences between the 30 m samples and the 60 m and 90 m samples ([@fig:fig_cap]).



```{r cap, fig.cap="Canonical analysis of principal coordinates (CAP) plot showing variation in brachyuran crab communities by sampling region and depth zone. Ellipses represent 95% confidence intervals for sample groups.", results="asis", fig.width=9, fig.height=6, dependson="cap-analysis"}
...
ggplot(pcoa,aes(x=LD1,y=LD2,fill=sample_group)) + 
  ...
  xlab(str_glue("Linear discriminant 1 [{scales::percent(cap_var$x,0.1)}]")) +
  ylab(str_glue("Linear discriminant 2 [{scales::percent(cap_var$y,0.1)}]")) +
  plotz_theme()
cat("{#fig:fig_cap}")
```
...
print_table(rda_table,"rda_table","Significant terms in the dbRDA analysis of crab community variation across the Hawaiian Archipelago",hdr_case="none") 
```